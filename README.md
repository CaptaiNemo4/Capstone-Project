...
The 35 unit tests run without any data files. The integration test requires `panel_clean.csv` and `argentina_bonds.csv` in the repo root and will be automatically skipped with a `SKIPPED` notice if they are absent — all other tests still pass.

To run the integration test explicitly:

```bash
pytest tests/test_pipeline.py -v -k integration
```
...
|---------|----------------------|
| `ModuleNotFoundError` | Run `pip install -r requirements.txt` again; check you are in the correct virtual environment |
| `python --version` shows < 3.9 | Upgrade Python; the `dict \| None` return type hint requires 3.9+ |
| Integration test fails on notebook execution | Check the full stderr output — usually a missing file path or column name mismatch; run the notebook cell-by-cell in Jupyter to find the failing step |
| `panel_analysis.csv` not created | `data_pipeline.ipynb` hit an error mid-run; open it in Jupyter and run cells one at a time |
| Convexity assertion error in pipeline step 2 | Your panel has a convexity column that is still bimodal after rescaling; check for a third scale variant in your Refinitiv export |
| Analysis notebook raises `FileNotFoundError` | Run `data_pipeline.ipynb` first — `panel_analysis.csv` must exist before the analysis notebook is opened |
