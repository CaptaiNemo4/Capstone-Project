...
# 4. Run the test suite (should report: 44 passed)
pytest tests/test_pipeline.py -v

# 5. Build the clean panel  ->  produces panel_analysis.csv
jupyter nbconvert --to notebook --execute data_pipeline.ipynb \
    --output data_pipeline_executed.ipynb

# 6. Run the analysis  ->  produces figures in output_convexity/
jupyter nbconvert --to notebook --execute convexity_analysis_clean.ipynb \
    --output convexity_analysis_executed.ipynb
```

Requires Python 3.9 or later. Steps 5 and 6 must run in order:
`convexity_analysis_clean.ipynb` consumes the `panel_analysis.csv` produced
by `data_pipeline.ipynb`. Each section below documents these steps in more
detail.
...
```
.
├── data_pipeline.ipynb             # Step 1 – data cleaning & harmonization
...
├── panel_analysis.csv              # Example output of Step 1 (regenerated on run)
...
**`panel_analysis.csv`** — the harmonized, analysis-ready panel. Columns:
...
Run `data_pipeline.ipynb` first. `convexity_analysis_clean.ipynb` depends on its output (`panel_analysis.csv`). Run both from the repository root so the notebooks find their input CSVs.

---

## Running the tests

Run from the repository root (the integration test executes
`data_pipeline.ipynb` via a relative path):

```bash
pytest tests/test_pipeline.py -v
```

The full suite is 44 tests and should report `44 passed`.
...
- Convexity scale-harmonization logic (bimodal detection and rescaling)
